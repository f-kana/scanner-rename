...
EnterWorktree または git worktree add の前に、必ず `git fetch origin main --prune` を実行して origin/main を最新にすること。PR マージによりローカルの main が遅れていることが多く、古いベースからブランチが切られる問題を防ぐ。
