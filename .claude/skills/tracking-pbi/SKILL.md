...
2. `.kiro/specs/{feature-name}/` の成果物の有無
   - feature-name の特定: `ls .kiro/specs/` で一覧を取得し、PBI の説明文と照合して特定する
...
3. `.claude/worktrees/` または `.worktrees/` 以下のディレクトリ名
   - PBI ID や feature 名に一致する worktree が存在すれば、進行中の可能性が高い
   - worktree の存在だけで断定せず、tasks.md の完了率と併せて判断する

3 ソースを総合して回答する。例:
> `docs/product-backlog.md` は `[ ]` ですが、`.kiro/specs/secret-scanning/tasks.md` が存在し、
> `.claude/worktrees/` に対応する worktree もあるため、Tasks 定義済み・実装前の状態です。
