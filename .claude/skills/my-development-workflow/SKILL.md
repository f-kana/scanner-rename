...
## Step 4: PBI クローズアウトノート

`tracking-pbi` で `[x]` にマークした後、以下を実行する。

### 詳細セクションの確認と移動

`docs/product-backlog.md` の `# PBI詳細` セクションに当該 PBI の記述があれば:

1. `docs/pbi-notes/<pbi-id>.md` を新規作成する（例: `docs/pbi-notes/ph0-026.md`）
2. 以下のテンプレートで **WHY 中心** のノートを書く:
...
cc-sdd（SDD フロー）で実装した PBI は spec が記録を担うため、ノート作成は任意。
ただし、**実装中に非自明な判断・制約・却下案があった場合**はノートを残すことを推奨する。
