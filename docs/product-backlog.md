...
- [ ] **PBI-ph0-020** セキュリティ隔離モデルのドキュメント修正。security-notes.md / initial-context.md の「認証情報はホストのみ」記述を実態（named volume で ADC がコンテナ内に存在）に合わせて書き直す。ph1-001 のブロッカー。
- [ ] **PBI-ph0-021** ph0-012 レビュー指摘の軽微即修正バッチ（CLAUDE.md ワークフロー節の更新、--no-verify の deny 追加、パス誤記修正ほか）。
...
PreToolUse(Skill) フックからの `additionalContext` 注入が実際に機能しているかを検証する。

- skill-context-injector.sh は UserPromptExpansion と PreToolUse の両イベントで同一の JSON 形式（`hookSpecificOutput.additionalContext`）を出力しているが、PreToolUse でこの形式が有効かは Claude Code の公式ドキュメントで裏取りできていない
- 検証方法の例: context ファイルにマーカー文字列を仕込み、モデル起動の Skill 呼び出し（ユーザーの `/skill` 入力ではなく）でマーカーがコンテキストに現れるかを確認する
- 機能していなければ、PreToolUse 側の出力形式を修正するか、UserPromptExpansion のみに一本化する
