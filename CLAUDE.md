...
- このプロジェクトは VS Code DevContainer 上で動作している（作業ディレクトリが `/workspaces/` 以下であることが目印）
- 日本語を使う
...
- プロンプト中に不明な固有名詞（ツール名・ライブラリ・コマンド名）が含まれる場合、ユーザーに確認する前に WebSearch で調査すること。調査しても判断できない場合のみ確認する。
