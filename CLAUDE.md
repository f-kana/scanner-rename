...
このプロジェクトは [gotalab/cc-sdd](https://github.com/gotalab/cc-sdd) v3 による Kiro-style Spec-Driven Development で開発する。「cc-sdd」には Spex 系と呼ばれる別系統も存在するが、本プロジェクトでは Kiro 系（`.kiro/` ディレクトリ、`/kiro-*` スキル）のみを使用する。ウェブ検索時に Spex 系の情報を混同しないこと。
